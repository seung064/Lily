        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateTable(
                ...
                columns: table => new
                {
                    ...
                    CountryNum = table.Column<int>(type: "INTEGER", nullable: true)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Product", x => x.ProductNum);
                    table.ForeignKey(
                        name: "FK_Product_Countries_CountryNum",
                        column: x => x.CountryNum,
                        principalTable: "Countries",
                        principalColumn: "CountryNum");
                });
            ...
            migrationBuilder.InsertData(
                table: "Product",
                columns: new[] { "ProductNum", "CountryNum", "Description_P", "ProductName", "Receipt" },
                values: new object[,]
                {
                    { 1, null, "고온, 고압의 핵융합 에너지 결정. 무기 코어나 동력 장치에 사용됨", "플라즈마 하트(Plasma Heart)", "화염정광 + 심해석 + 풍정석 + 암철석" },
                    { 2, null, "공중 부양 능력이 있는 에너지 광물. 공중 장비나 비행체에 쓰임", "천공핵(Skycore Crystal)", "용석탄 + 청류정수 + 공명결정 + 석기정" },
                    { 3, null, "대지와 불의 융합체로, 강력한 지진 및 폭발 효과를 가진 전술 병기 코어", "지화탄(Geomagma Capsule)", "발화진주 + 바다수정 + 회오리 플라스크 + 진토금" },
                    { 4, null, "원소의 핵심 에너지 덩어리. 고급 마법 무기나 장비에 필수", "에테르 코어(Aether Core)", "용석탄 + 청류정수 + 풍정석 + 암철석" },
                    { 5, null, "강한 자기력을 가진 금속 합성체. 방어구 또는 에너지 실드용", "마그네실(Magnesyl Alloy)", "화염정광 + 심해석 + 공명결정 + 진토금" },
                    { 6, null, "냉각 및 연료 안정화에 사용되는 특수 결정. 에너지 장치 필수 자원", "플루오라이트(Fluorite X)", "발화진주 + 청류정수 + 회오리 플라스크 + 석기정" },
                    { 7, null, "생명 에너지와 기술이 융합된 생체 동력 코어", "제네시움(Genesium)", "화염정광 + 바다수정 + 공명결정 + 석기정" }
                });
            ...
            migrationBuilder.InsertData(
                ...
                values: new object[,]
                {
                    { 1, 1, "무기 제작에 사용되는 고강도 금속 원석", "암철석" },
                    { 2, 1, "고열을 견디는 귀금속, 전도성이 뛰어남", "진토금" },
                    { 3, 1, "고대 생물의 화석에서 채굴되는 결정성 자원", "석기정" },
                    { 4, 2, "깊은 해저에서 추출되는 희귀 금속", "심해석" },
                    { 5, 2, "정제된 고순도 수자원, 생명 유지나 에너지 전환에 사용", "청류정수" },
                    { 6, 2, "수중 생명체 뼈대에서 채취한 반투명 결정체", "바다수정" },
                    { 7, 3, "대기 중에서 응결되는 가벼운 결정, 부유 장치 제작에 활용", "풍정석" },
                    { 8, 3, "고속 회전에너지로 저장된 압축 에너지 컨테이너", "회오리 플라스크" },
                    { 9, 3, "대기 진동과 공명하는 음파 기반의 마력 결정", "공명결정" },
                    { 10, 4, "용암 지대에서 채굴되는 고열 금속 결정", "화염정광" },
                    { 11, 4, "천연 용암석으로 이루어진 고열 연료 자원", "용석탄" },
                    { 12, 4, "마그마 생물의 분비물로 형성된 진귀한 에너지 진주", "발화진주" }
                });
